 ...
 */
  ...
  //  }
    ...
    for (var i = 0; i < trial.stimuli.length; i++) {
      var coords = random_coordinate(trial.sort_area_width - trial.stim_width, trial.sort_area_height - trial.stim_height) += '<img '+
        'src="'+trial.stimuli[i]+'" '+
    ...
    var draggables = display_element.querySelectorAll('.jspsych-btn');
    ...
    display_element.querySelector('#jspsych-free-sort-done-btn').addEventListener('click', function(){
      ...
      var final_locations = [];
      var button_presses = display_element.querySelectorAll('.jspsych-btn');
      for(var i=0; i<button_presses.length; i++){
        final_locations.push({
          "src": matches[i].dataset.src,
          "x": parseInt(button_presses[i].style.left),
          "y": parseInt(button_presses[i].style.top)
        });
      }
